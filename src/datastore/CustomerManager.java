...
public class CustomerManager {
    ...
    * 			: the new identity number to give to the Customer
    * @param customerNickname
    * 			: the new nickname to give to the Customer
    * @param customerSerialNumber
    * 			: the new serial number to give to the Customer
    * @param customerPhoneNumber
    * 			: the new phone number to give to the Customer
    * @param village
    * 			: the new village of the Customer
    ...
    */
	public static void updateCustomerAttributes(
		...
		try {
			...
			customer.setCustomerSerialNumber(customerSerialNumber);
    ...
    */
	...
	/**
    * Update Item Version.
    * Updates the given Customer's Item Version by 1 in the datastore.
    * @param email
    * 			: the email of the Customer whose attributes will be updated
    */
	public static void updateItemVersion(Email email) {	
		...
		try {
			...
			log.info("Customer \"" + email.getEmail() + 
					"\"'s Item version updated in datastore.");
		}
		finally {
			if (tx.isActive()) {
				tx.rollback();
	...
	/**
    * Update Customer Service Version.
    * Updates the given Customer's Service Version by 1 in the datastore.
    * @param email
    * 			: the email of the Customer whose attributes will be updated
    */
	public static void updateServiceVersion(Email email) {	
		...
		try {
			...
			tx.commit();
			log.info("Customer \"" + email.getEmail() + 
					"\"'s Service version updated in datastore.");
		}
		finally {
			if (tx.isActive()) {
				tx.rollback();
